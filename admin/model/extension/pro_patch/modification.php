 ...
 *
 */
class ModelExtensionProPatchModification extends Model
{
    public function modificationHandler($codename, $status, $extra = False)
    {
            ...
            if ($xml) {
                try {
                    ...
                    if ($code) {
                        ...
                        if (isset($modification_info) && $modification_info) {
    ...
    public function getModificationId($xml)
    {
        ...
        if ($code || $name) {
            if (VERSION <= '2.0.0.0') {
                $search_code = ($name) ?: $code;
                $query = $this->db->query("SELECT * FROM `". DB_PREFIX ."modification` WHERE `name` = '" . $this->db->escape($search_code) . "'");
            } else {
                $search_code = ($code) ?: $name;
                $query = $this->db->query("SELECT * FROM `". DB_PREFIX ."modification` WHERE `code` = '" . $this->db->escape($search_code) . "'");
            }
    ...
    private function getFilePath($xml)
    {
        $file = str_replace("system/", "", DIR_SYSTEM).$xml;

        if (!file_exists($file)) {
            $file =  DIR_SYSTEM."library/pro_hamster/install/{$xml}";
        }
